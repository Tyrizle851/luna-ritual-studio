...
// ============================================================
// FULL DESIGN SPEC SYSTEM - Ported from src/types/design-spec.ts
// ============================================================
...
type LayoutArchetype =
  | "centered-serenity" | "vertical-flow" | "floating-cluster" | "asymmetric-balance"
  | "arc-flow" | "golden-spiral" | "botanical-frame" | "minimal-horizon"
  | "radiant-center-burst" | "soft-anchor-left" | "soft-anchor-right" | "gentle-column"
  | "pebble-scatter" | "circle-harmony" | "prayer-stack" | "ribbon-drift"
  | "editorial-grid-luxe" | "calm-waterfall" | "sacred-geometry" | "breath-space-minimal";

type EnergyLevel = "soft" | "supportive" | "direct" | "intense";
type AccentType = "minimal" | "organic" | "botanical" | "geometric" | "textured" | "playful" | "arrows" | "celestial";
type TypographyStyle = "serif" | "sans" | "display" | "script";
...
// ============================================================
// THEME REGISTRY - 15 Themes with full data
// ============================================================

const THEME_REGISTRY: Record<ThemeSlug, ThemeDefinition> = {
  confidence: {
...
// ============================================================
// MOOD REGISTRY - 13 Moods with accents and layouts
// ============================================================

const MOOD_REGISTRY: Record<MoodSlug, MoodDefinition> = {
  minimalist: {
    accents: ["minimal"],
...
// ============================================================
// LAYOUT DESCRIPTIONS - 20 layouts
// ============================================================

const LAYOUT_DESCRIPTIONS: Record<LayoutArchetype, string> = {
...
// ============================================================
// ACCENT DESCRIPTIONS - 8 accent types
// ============================================================

const ACCENT_DESCRIPTIONS: Record<AccentType, string> = {
  minimal: "subtle single-line geometric marks, thin strokes, negative space",
  organic: "flowing curves, gentle wave patterns, soft irregular shapes",
  botanical: "delicate leaf sketches, vine tendrils, flower line art",
  geometric: "clean circles, triangles, hexagons, precise patterns",
  textured: "paper grain, gentle noise, handmade imperfection",
  playful: "soft dots, gentle swirls, whimsical doodles",
  arrows: "directional marks, subtle pointing guides, flow indicators",
  celestial: "crescent moons, stars, cosmic dust, soft glow"
};
...
function mapCategoryToTheme(category: string): ThemeSlug {
  const mapping: Record<string, ThemeSlug> = {
    'rest': 'peace',
    'peace': 'peace',
    'growth': 'renewal',
    'self-love': 'gratitude',
  ...
  return mapping[category.toLowerCase()] || 'peace';
}

function selectWeightedMood(): MoodSlug {
  const moods = Object.entries(MOOD_REGISTRY) as [MoodSlug, MoodDefinition][];
  const totalWeight = moods.reduce((sum, [_, m]) => sum + m.weight, 0);
  let random = Math.random() * totalWeight;
  
  for (const [slug, mood] of moods) {
    random -= mood.weight;
    if (random <= 0) return slug;
  }
  return 'minimalist';
}

function selectCompatibleLayout(themeData: ThemeDefinition, moodData: MoodDefinition): LayoutArchetype {
  const compatible = themeData.defaultLayouts.find(l => moodData.allowedLayouts.includes(l));
  if (compatible) return compatible;
  
  // Fallback: pick random from mood's allowed layouts
  const moodLayouts = moodData.allowedLayouts;
  return moodLayouts[Math.floor(Math.random() * moodLayouts.length)] || themeData.defaultLayouts[0];
}

function getTypographyDescription(typo: TypoSet, variant: number): string {
  const headlines: Record<TypographyStyle, string[]> = {
    serif: ['elegant thin serif (Didot)', 'transitional serif with warmth', 'contemporary luxury serif', 'refined modern serif'],
    sans: ['clean geometric sans', 'modern grotesque sans', 'humanist sans with warmth', 'minimal Swiss sans'],
    display: ['artistic display type', 'expressive headline face', 'bold statement type', 'decorative display'],
    script: ['flowing calligraphic script', 'elegant brush lettering', 'refined cursive', 'hand-lettered style']
  };
  return headlines[typo.headline][variant % 4];
}

function getEnergyDescription(energy: EnergyLevel): string {
  const descriptions: Record<EnergyLevel, string> = {
    soft: 'gentle, quiet, whisper-like presence',
    supportive: 'warm, encouraging, nurturing embrace',
    direct: 'clear, confident, purposeful statement',
    intense: 'powerful, bold, commanding presence'
  };
...
function buildDesignSpec(
  ...
  // Random mood selection (weighted)
  const mood = selectWeightedMood();
  const moodData = MOOD_REGISTRY[mood];
  
  // Get compatible layout
  const layout = selectCompatibleLayout(themeData, moodData);
  
  // Generate variant knobs for controlled randomness
  const styleVariant = Math.floor(Math.random() * 4);
  const accentVariant = Math.floor(Math.random() * 3);
  const paletteVariant = Math.floor(Math.random() * 3);
  const textureVariant = Math.floor(Math.random() * 2);
  
  // Gradient effect (35% chance)
  const useGradient = Math.random() < 0.35;
  
  // Use provided title or select from lexicon
  const headline = title || themeData.headlineLexicon[Math.floor(Math.random() * themeData.headlineLexicon.length)];
  
  // Use provided phrases or select from templates
  const phrases = supportingPhrases.length > 0 
    ? supportingPhrases 
    : themeData.phraseTemplates[Math.floor(Math.random() * themeData.phraseTemplates.length)];
  ...
  return {
    ...
    accents: moodData.accents,
    typography: themeData.typography,
    headline,
    phrases,
    useGradient,
    variants: { styleVariant, accentVariant, paletteVariant, textureVariant }
  };
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Build full design spec
    const spec = buildDesignSpec(category, title, supportingPhrases);
    console.log(`Design spec: theme=${spec.theme}, mood=${spec.mood}, layout=${spec.layout}, useGradient=${spec.useGradient}`);
